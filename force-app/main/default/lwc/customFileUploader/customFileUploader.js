...
import uploadToS3 from '@salesforce/apex/LWCExampleController.fileUploadTest';
...
export default class CustomFileUploader extends LightningElement {
    ...
    saveToFile() {
        saveFile({ idParent: this.recordId, strFileName: this.file.name, base64Data: this.fileContents})
        .then(result => {
    ...
    uploadData() {
        ...
        uploadToS3({idParent: this.recordId,strFileName: this.file.name, base64Data: encodeURIComponent(this.fileContents)})
        .then(result => {
            window.console.log('result ====> ' +result);
